...
class MountainCarQAgent:
    def __init__(self, action_bucket = 0.2, num_episodes=1000, min_lr=0.1, min_epsilon=0.1, discount=1.0, decay=25):
        ...
        self.Q1_table = np.zeros((num_states[0], num_states[1], len(self.actions)))
    ...
    def update_q(self, state, action, reward, new_state):
        ...
        if np.random.random() < 0.5:
            ...
            actual_reward = self.Q1_table[state[0], state[1], indx_action]

            self.Q1_table[state[0], state[1], indx_action] += \
                self.learning_rate * (
                    reward + self.discount * np.max(self.Q1_table[new_state[0], new_state[1]]) - self.Q_table[
                        state[0], state[1], indx_action])

            updated_reward = self.Q1_table[state[0], state[1], indx_action]

            if updated_reward > actual_reward:
                self.actions[indx_action] = action[0]

            return self.Q1_table[state[0], state[1], indx_action]
